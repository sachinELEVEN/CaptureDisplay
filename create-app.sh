# ---------------------------------------
# Step 1: Clear dist folder and create CaptureDisplayX.app
# ---------------------------------------
...
#pyinstaller --noconsole --name CaptureDisplayX --icon=./assets/CaptureDisplayX.icns /Users/sachinjeph/Desktop/CaptureDisplay/main.py
#But for some reason creating the app using the below flag allows notarisation else notarisation does not work
#pyinstaller --onefile --windowed ./main.py --name "CaptureDisplayX" --icon=./assets/CaptureDisplayX.icns
rm -r dist
rm -r build
#create a bundle identifer from appstoreconnect and use that in the spec file
pyinstaller CaptureDisplayX.spec
cd dist
find CaptureDisplayX.app -name .DS_Store -delete
cd ..
...
# ---------------------------------------
# Step 2: Signing the CaptureDisplayX.app
# ---------------------------------------
#Signing needs to happen with a certificate- if you use developer id certificate like the one here the notarization does not work, we need to use self signed certificate like 'CaptureDisplayX'. Example developer application id certificate -> EC24DE91843FE9267B360FA70CAFAF873E92AC72 "Developer ID Application: sachin jeph (34XRA32US9)"
...
# codesign --deep --force --verbose -s "CaptureDisplayX" "./dist/CaptureDisplayX.app"
#we must sign with Developer ID Application certificate in order to notarize the app
#verify codesign -> codesign -vvv --deep --strict ./dist/CaptureDisplayX.app
codesign -s "EC24DE91843FE9267B360FA70CAFAF873E92AC72" -v --deep --force --timestamp --entitlements entitlements.plist -o runtime "./dist/CaptureDisplayX.app"
...
create-dmg  --volname "CaptureDisplayX" --volicon "/Users/sachinjeph/Desktop/CaptureDisplay/assets/CaptureDisplayX.ico" --icon-size 100 --app-drop-link 425 120 "./dist/CaptureDisplayX.dmg" "./dist/CaptureDisplayX.app/"
# ---------------------------------------
# Step 4: Signing the CaptureDisplayX.dmg
# ---------------------------------------
#Signing the dmg
echo "Signing DMG..."
codesign -s "EC24DE91843FE9267B360FA70CAFAF873E92AC72" -v --deep --force --timestamp --entitlements entitlements.plist -o runtime "./dist/CaptureDisplayX.dmg"
...
# xcrun notarytool submit "./dist/CaptureDisplayX.dmg" --keychain-profile "CaptureDisplayX" --wait
# #Staple the dmg, this allows for app verification without internet access on user's side
# xcrun stapler staple "./dist/CaptureDisplayX.dmg"
#Validate stapler step
#xcrun stapler validate "./dist/CaptureDisplayX.dmg"
#To verify if a dmg is notarised or not
# spctl -a -vvv -t install "./dist/CaptureDisplayX.dmg"
#To see history of your notarization request run the below command
# xcrun notarytool history --keychain-profile "CaptureDisplayX"
#To get the detailed log of notarization step use the below commnad
#xcrun notarytool log "./dist/CaptureDisplayX.dmg" --keychain-profile "CaptureDisplayX"
...
#xcrun notarytool log 8e03c9e3-219f-48be-a22d-4ba4846490c3 --keychain-profile "CaptureDisplayX"
